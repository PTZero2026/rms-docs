...
- Ánh xạ feature ↔ module backend là BẮT BUỘC giữ (xem AGENTS.md §3):
...
`docs/features/` = nguồn sự thật nghiệp vụ đã duyệt. `docs/epics/` = lớp Epic (E0–E3) gom feature
...
- Có test khớp `test-plan.md`; lint & build sạch (khi đã có code).
- Không phá ánh xạ feature↔module và 5 luật bất biến ở trên.
