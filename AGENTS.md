...
| `packages/domain-types` | Enum/model dùng chung (vd trạng thái `DeTai`) | Type chia sẻ |
...
2. **Vòng đời `DeTai`** chuyển trạng thái qua domain service dùng chung — KHÔNG tự update enum ở màn hình.
3. **Hợp đồng API**: sửa ở `packages/api-contracts` rồi sinh type; không tự gõ type lệch hai phía.
4. **Audit**: mọi hành động đổi trạng thái nghiệp vụ ghi `NhatKyHeThong` (append-only).
