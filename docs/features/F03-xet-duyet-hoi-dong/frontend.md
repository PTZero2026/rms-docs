...
- **Ngữ cảnh:** Chủ nhiệm **không** tham gia chấm điểm; ở FE họ chỉ **theo dõi tiến trình và kết quả**
  ...
  - Xem trạng thái đề tài đang `DANG_XET_DUYET` / `DUYET` / `TU_CHOI`.
...
| FE-02 | Kết quả xét duyệt | Xem kết luận DUYET/TU_CHOI, điểm tổng hợp (nếu công khai) và nhận xét hội đồng (nếu công khai) |
...
- Là một tab/section trong màn chi tiết đề tài (F01). Chỉ hiển thị khi đề tài đã từng vào xét duyệt
  (`DANG_XET_DUYET` trở đi).
- Thành phần: chip trạng thái (`Đang xét duyệt` / `Đã duyệt` / `Từ chối`); timeline mốc:
  "Vào xét duyệt" (ngày tạo `DotDanhGia`), "Có kết luận" (ngày `DA_KET_LUAN`).
- Trạng thái:
...
- Hiển thị khi đề tài đã `DUYET` hoặc `TU_CHOI`.
- Thành phần:
  - Kết luận nổi bật: "Đề tài được duyệt" / "Đề tài không được duyệt".
  - `diemTongHop` — **chỉ hiển thị nếu** cấu hình công khai cho phép; nếu không, ẩn điểm.
  - Khối "Nhận xét hội đồng" — **chỉ hiển thị khi** `XET_DUYET.CONG_KHAI_NHAN_XET=true` (BR-11);
    danh tính người chấm ẩn (hiển thị "Thành viên hội đồng 1/2/..." hoặc gộp chung).
  - Với `DUYET`: ghi chú "đề tài sẽ được chuyển sang giai đoạn thực hiện" (F04).
- Trạng thái:
...
| Có kết luận DUYET | "Chúc mừng! Đề tài *X* đã được hội đồng thông qua." |
| Có kết luận TU_CHOI | "Đề tài *X* không được hội đồng thông qua." |
...
|----------|----------------------------------------------------------------|
| FE-01 | AC-01 (vào xét duyệt → hiển thị trạng thái `DANG_XET_DUYET`) |
| FE-02 | AC-03 (kết quả DUYET), AC-04 (kết quả TU_CHOI), liên quan BR-11 (công khai nhận xét) |
