...
- **Dữ liệu mẫu:**
  - Đợt `KG-2026-01` trạng thái `OPEN`, `startDate` ≤ hôm nay ≤ `endDate`, có `researchFieldIds` và
    `proposalTemplateId`.
  - Đợt `KG-2025-12` trạng thái `CLOSED` / hết hạn (`endDate` < hôm nay).
...
|-------|-------------|----------------|----------------|------------------|------|
| TC-01 | AC-01 | CN1 đăng nhập; đợt `KG-2026-01` `OPEN` | Tạo đề xuất mới trong đợt → lưu nháp | `ResearchProject` tạo ở `DRAFT`; `principalInvestigatorId`=CN1; có 1 `ProjectMember` `PRINCIPAL_INVESTIGATOR`; chưa có `projectCode` | Happy |
| TC-02 | AC-02 | Đề xuất `DRAFT` đủ trường bắt buộc; đợt `OPEN` còn hạn | Nhấn Nộp ở FE-06 | Chuyển `SUBMITTED`; sinh `projectCode` unique; set `submittedAt`; hồ sơ khóa sửa; có bản ghi `RESEARCH_PROJECT.SUBMIT` trong `AuditLog` | Happy |
| TC-03 | AC-03 | Đề xuất `DRAFT` thuộc đợt `KG-2025-12` đã hết hạn/`CLOSED` | Nhấn Nộp | Bị chặn; giữ `DRAFT`; thông báo "Đã hết hạn nộp của đợt" | Biên/Lỗi |
...
| TC-07 | AC-07 | CV1; đề xuất `SUBMITTED` trong đợt còn hạn | BO-04: nhập `reason` → trả lại | Chuyển `DRAFT`; hồ sơ mở khóa; `reason` hiển thị cho CN1 ở FE-07; bản ghi `RESEARCH_PROJECT.RETURN_FOR_REVISION` | Happy |
| TC-08 | AC-08 | CV1; đề xuất `SUBMITTED` thuộc đợt đã hết hạn | BO-04: cố trả lại bổ sung | Hành động bị chặn/vô hiệu; thông báo đợt đã hết hạn | Biên/Lỗi |
...
| TC-10 | AC-10 | CV1; ≥2 đề xuất `SUBMITTED` hợp lệ trong đợt | BO-05: chọn & Chốt danh sách | Danh sách đánh dấu sẵn sàng xét duyệt; bàn giao F03; F01 **không** tự đổi sang `UNDER_REVIEW` | Happy |
...
| TC-12 | AC-02 | Hai phiên nộp gần như đồng thời 2 đề xuất khác nhau trong cùng đợt | Nộp song song | Hai `projectCode` khác nhau, không trùng (BR-07) | Biên |
...
| TC-14 | AC-02 | Đề xuất `DRAFT`, chọn `researchFieldId` không thuộc `researchFieldIds` của đợt | Nộp | Bị chặn (BR-03); thông báo lĩnh vực không hợp lệ với đợt | Biên/Lỗi |
...
- **Dữ liệu rỗng:** danh sách đề xuất trống (FE-01/BO-01); không có đợt `OPEN` khi tạo mới; đề xuất
...
- **F02 (Đợt kêu gọi):** đổi đợt sang `CLOSED`/`CANCELLED` phải chặn nộp mới và trả lại (BR-01, BR-06).
