...
Vào từ Portal người dùng (FE): menu "Đề xuất của tôi" hoặc từ một đợt kêu gọi đang `MO` → "Tạo đề
...
| FE-03 | Bước thành viên | Thêm/sửa/xóa thành viên đề tài (`ThanhVienDeTai`). |
| FE-04 | Bước dự toán kinh phí | Nhập `kinhPhiDeXuat` và `thoiGianThucHien`. |
| FE-05 | Bước tài liệu đính kèm | Tải lên/xóa file (`TaiLieuDinhKem`). |
| FE-06 | Xem lại & nộp | Tổng hợp, kiểm tra hợp lệ, nộp (`NHAP` → `DA_NOP`). |
| FE-07 | Chi tiết đề xuất (chỉ đọc) | Xem hồ sơ đã nộp, trạng thái, `maDeTai`, lý do trả lại. |
...
- **Bố cục:** bảng các đề xuất của người dùng (cột: `maDeTai` hoặc "—" khi chưa nộp, tên, đợt kêu
  gọi, lĩnh vực, trạng thái, cập nhật gần nhất). Nút **Tạo đề xuất**.
- **Tạo đề xuất:** mở dialog chọn **đợt kêu gọi đang `MO`** (chỉ liệt kê đợt `MO` còn hạn). Chọn
  xong → tạo `DeTai` ở `NHAP`, điều hướng tới FE-02. → AC-01.
- **Badge trạng thái:** `NHAP` (xám "Nháp"), `DA_NOP` (xanh "Đã nộp"), `HUY` (đỏ "Đã hủy").
- **Trạng thái rỗng:** chưa có đề xuất → minh họa + CTA "Tạo đề xuất". Nếu không có đợt nào `MO`:
...
- **Stepper:** Thông tin chung → Thuyết minh → Thành viên (FE-03) → Dự toán (FE-04) → Tài liệu
  (FE-05) → Xem lại & nộp (FE-06). Cho phép nhảy bước; mọi bước **Lưu nháp** giữ `NHAP`.
- **Thông tin chung:** `ten` (bắt buộc), `linhVucId` (chọn trong lĩnh vực của đợt — BR-03),
  `donViChuTriId`, `tomTat`.
- **Thuyết minh:** render động theo `bieuMauThuyetMinhId` của đợt (lưu vào `thuyetMinh` jsonb).
  Đánh dấu trường bắt buộc bằng dấu *.
- **Chỉ sửa khi `NHAP`:** nếu đề xuất ở `DA_NOP`, form chuyển chế độ chỉ đọc và hiện banner "Hồ sơ
...
- Danh sách thành viên hiện có; dòng chủ nhiệm gắn cố định vai trò `CHU_NHIEM`, không xóa được
  (BR-04). Thêm thành viên: tìm `NguoiDung` theo tên/email, chọn `vaiTroTrongDeTai`
  (`THANH_VIEN`/`THU_KY`), nhập `nhiemVu`.
- **Validate:** không cho thêm trùng người dùng → cảnh báo "Thành viên đã có trong đề tài" (BR-08)
  → AC-09.

### FE-04 — Bước dự toán kinh phí
- `kinhPhiDeXuat` (VND, định dạng nhóm nghìn, số nguyên ≥ 0 — BR-09), `thoiGianThucHien` (số tháng,
  > 0). Validate tại chỗ: số âm/0/không phải số → báo lỗi inline.

### FE-05 — Bước tài liệu đính kèm
- Kéo-thả/chọn file → tải lên object storage, tạo `TaiLieuDinhKem` (`loaiDoiTuong='DeTai'`). Hiển
  thị tên, dung lượng, loại; cho xóa khi `NHAP`. Báo lỗi nếu vượt dung lượng/định dạng không hỗ
...
- **Nộp:** xác nhận → gọi API nộp. Backend là nơi quyết định (validate lại). Thành công → chuyển
  `DA_NOP`, hiển thị `maDeTai` và `ngayNop` → AC-02. Nếu backend báo hết hạn (AC-03) hoặc thiếu
  trường (AC-04): hiển thị lỗi tương ứng, giữ `NHAP`.

### FE-07 — Chi tiết đề xuất (chỉ đọc)
- Xem hồ sơ đã nộp: `maDeTai`, trạng thái, `ngayNop`, thuyết minh, thành viên, dự toán, tài liệu.
- **Lý do trả lại:** khi đề xuất bị trả về `NHAP`, hiển thị banner nổi bật với `lyDo` của chuyên
  viên và nút "Sửa & nộp lại" (mở FE-02 ở chế độ sửa) → AC-07. `maDeTai` giữ nguyên ở lần nộp lại
  → AC-11.
- **Hủy đề xuất:** nút "Hủy đề xuất" cho `NHAP`/`DA_NOP` (trước xét duyệt), yêu cầu nhập lý do
  (BR-10).
...
| Nộp thành công | "Đã nộp đề xuất. Mã đề tài: {maDeTai}." |
...
| Bị trả lại bổ sung | Banner: "Hồ sơ được trả lại để bổ sung. Lý do: {lyDo}." (AC-07) |
| Không có quyền | "Bạn không có quyền truy cập đề xuất này." (AC-06) |

- **Trạng thái rỗng:** danh sách trống, không có đợt `MO`, chưa có thành viên/tài liệu — đều có
  minh họa + hướng dẫn.
