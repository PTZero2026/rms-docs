...
**Kết quả mong đợi:**
- Mọi đề xuất nằm trong một đợt kêu gọi đang `OPEN`, dữ liệu chuẩn hóa theo biểu mẫu, truy vết được.
...
- **Trong phạm vi:**
  - Chủ nhiệm tạo/sửa đề xuất khi đợt kêu gọi `OPEN`: điền thuyết minh theo biểu mẫu của đợt, thêm
...
```mermaid
flowchart TD
    start([Đợt kêu gọi đang OPEN]) --> create["Chủ nhiệm tạo đề xuất<br/>(ResearchProject: DRAFT)"]
    ...
    draft --> edit
    save -->|Nộp| validate{"Đợt còn OPEN &amp; trong hạn<br/>+ đủ trường bắt buộc?"}
...
Diễn giải các bước:
1. **Tạo đề xuất:** chủ nhiệm chọn một đợt kêu gọi đang `OPEN`, hệ thống tạo `ResearchProject` ở `DRAFT`,
   `principalInvestigatorId` = người tạo, đồng thời tạo một `ProjectMember` vai trò `PRINCIPAL_INVESTIGATOR`.
2. **Soạn hồ sơ:** điền `name`, `researchFieldId` (thuộc lĩnh vực của đợt), `abstract`, `proposalDocument`
   (jsonb theo `proposalTemplateId` của đợt), `durationMonths`, `requestedBudget`; thêm thành
...
5. **Trả lại bổ sung:** nếu hồ sơ thiếu/sai và đợt còn hạn, chuyên viên trả về `DRAFT` kèm `reason`;
...
|-------|---------|-------|---------|
| BR-01 | Chỉ nộp khi đợt `OPEN` & còn hạn | `DRAFT` → `SUBMITTED` chỉ thực hiện khi `ProposalCall.status = OPEN` và thời điểm nộp trong khoảng `[startDate, endDate]`. | Quá `endDate` hoặc đợt `CLOSED/CANCELLED` → chặn nộp. |
| BR-02 | Đủ trường bắt buộc của biểu mẫu | Phải đủ trường bắt buộc của `ResearchProject` (`name`, `researchFieldId`, `durationMonths`, `requestedBudget`) và mọi trường bắt buộc trong `proposalDocument` theo `proposalTemplateId` của đợt mới được nộp. | Validate tại backend; FE chỉ phản ánh sớm. |
| BR-03 | Lĩnh vực hợp lệ với đợt | `ResearchProject.researchFieldId` phải thuộc `ProposalCall.researchFieldIds` của đợt nộp. | Nếu đợt không giới hạn lĩnh vực thì bỏ qua. |
...
| BR-06 | Trả lại mới mở khóa sửa | Sau `SUBMITTED`, hồ sơ chỉ sửa tiếp được khi chuyên viên trả lại bổ sung (`SUBMITTED` → `DRAFT`) kèm `reason`, và chỉ khi đợt còn hạn. | Chuyển lùi bắt buộc có `reason` (data-model §3). |
...
- **`ResearchProject`** (§4.3): trục chính của feature. F01 dùng `projectCode` (sinh khi nộp), `name`,
  ...
  biểu mẫu đợt), `requestedBudget` (bigint VND), `durationMonths` (int tháng), `status`
...
|----|-----|-----------|-----------------|----|
| `DRAFT` | `SUBMITTED` | Đợt `OPEN` & còn hạn, đủ trường bắt buộc | Chủ nhiệm | BR-01, BR-02 |
...
- **AC-01** (happy — tạo & lưu nháp) — Given chủ nhiệm đăng nhập và một đợt kêu gọi đang `OPEN`,
...
- **AC-02** (happy — nộp hợp lệ) — Given một đề xuất `DRAFT` đủ trường bắt buộc trong đợt `OPEN` còn
...
- **AC-03** (biên — quá hạn nộp) — Given một đề xuất `DRAFT` và thời điểm hiện tại sau `endDate`
  của đợt (hoặc đợt đã `CLOSED`), When chủ nhiệm nộp, Then hệ thống chặn, giữ `DRAFT` và báo lỗi
...
- **AC-07** (happy — chuyên viên trả lại bổ sung) — Given chuyên viên QL KHCN và một đề xuất
  `SUBMITTED` trong đợt còn hạn, When trả lại bổ sung kèm `reason`, Then `ResearchProject` chuyển `DRAFT`, hồ sơ
  mở khóa cho chủ nhiệm sửa, `reason` hiển thị cho chủ nhiệm và ghi `AuditLog` (BR-06).
- **AC-08** (biên — trả lại khi hết hạn) — Given một đề xuất `SUBMITTED` mà đợt đã hết hạn nộp,
...
- **AC-10** (happy — chốt sang xét duyệt) — Given các đề xuất `SUBMITTED` hợp lệ trong một đợt,
...
**Phụ thuộc:**
- **F02 — Đợt kêu gọi:** cần đợt ở `OPEN`, `startDate`/`endDate`, `researchFieldIds`, `proposalTemplateId`.
...
**Rủi ro / cần làm rõ:**
- **Định dạng `projectCode`:** chốt quy tắc đánh số (theo đợt? theo năm?) và chống trùng khi nộp đồng
  thời (cần khóa/sequence ở backend) — BR-07.
- **Biểu mẫu đổi giữa đợt:** nếu biểu mẫu thay đổi sau khi có đề xuất `DRAFT`, cần quy tắc version
...
- **Hết hạn khi đang nháp:** đề xuất `DRAFT` chưa nộp khi đợt `CLOSED` sẽ không nộp được — cần thông
