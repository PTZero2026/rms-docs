...
- **Chủ nhiệm đề tài** (FE): xem dự toán vs thực chi của đề tài mình, giải trình khoản chi, đính chứng từ.
  **Không** chạy đối soát, **không** đổi `reconciliationStatus`, **không** sửa dự toán (BR-08).
...
| FE-02 | Chi tiết giao dịch | Danh sách `BudgetTransaction`, trạng thái đối soát; giải trình & đính chứng từ |

## 3. Mô tả màn hình & thao tác

### FE-01 — Tổng quan kinh phí đề tài
- Bảng theo khoản mục: `estimatedAmount`, tổng đã chi, còn lại, % giải ngân; tô cảnh báo khi chi vượt/đạt ngưỡng.
...
- Với mỗi giao dịch chi: chủ nhiệm có thể **giải trình** (nhập diễn giải) và **đính chứng từ** (`Attachment`).
