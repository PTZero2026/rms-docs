...
## Quyết định
Dùng chung bộ thực thể `EvaluationCommittee`, `CommitteeMember`, `CriteriaSet`/`EvaluationCriterion`, `EvaluationRound`,
`ScoreSheet`, `CriterionScore`. Phân biệt mục đích bằng trường `type = PROPOSAL_REVIEW | ACCEPTANCE` trên
`EvaluationCommittee`, `CriteriaSet` và `EvaluationRound`.

## Phương án đã cân nhắc
- **A — Mô hình dùng chung, phân biệt bằng `type` (chọn):** không trùng lặp; cải tiến quy trình chấm
...
- Thống kê hội đồng/điểm số làm một lần, lọc theo `type`.
